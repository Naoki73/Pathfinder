    ...
    while not open_set.empty():
        ...
        for neighbor in current.neighbors:
            ...
            if temp_g_score < g_score[neighbor]:
                #if we found a better path then update to this optimal path
...
def draw_grid(win, rows, width):
    ...
    for i in range(rows):
        ...
        #we are going to multiply the index of the row by the gap, to tell us where to draw the grid line


def draw(win, grid, rows, width):
    win.fill(WHITE)
    #fills entire grid in White at the start
    ...
    #take what has been drawn, put it on the display
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if pygame.mouse.get_pressed()[0]: # left
                ...
                node = grid[row][col]
                #give us row,column that we clicked on

                if not start and node != end:
                #not let users draw the start and end on top of each other
                    start = node
                    start.make_start()
